...
双击 `dashboard.bat` 可以打开本地可视化控制台。控制台会读取所选账号看到的首页帖子，并按页面最新顺序列出；点击某条帖子后，程序会在该账号自己的 Chrome 窗口中打开。

控制台地址是 `http://127.0.0.1:8790`。它和普通启动共用同一套账号目录，因此同一个账号不要同时在 `launch.bat` 和 `dashboard.bat` 中打开。

每条帖子有两个操作：只用当前账号打开，或让号池中的所有账号都打开同一条帖子。后者只打开页面，不会自动发布评论；评论需要在各个窗口中人工确认。
