...
2. 双击 `launch.bat`。
3. 首次运行按提示输入账号 Markdown 文件、窗口数量、起始账号序号和 Cookie 数据目录。
4. 首次登录或 Cookie 失效时输入共享短信验证码；已有有效 Cookie 时可以直接留空。验证码只通过当前进程环境变量传递，不写入配置文件。
5. 关闭 Chrome 窗口后，程序才会退出。再次运行会复用原账号目录中的 Cookie。
